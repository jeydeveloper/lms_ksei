...
class Training extends Base {
	...
	function showlist($catid = 0)
	{
		...
		{
		    $list[$i]->training_created_date = substr($list[$i]->training_created_date, 0, 4)."-".substr($list[$i]->training_created_date, 4, 2)."-".substr($list[$i]->training_created_date, 6, 2);
...
//echo $sortby.";".$orderby;
	...
	function examintro($id)
	{
		...
		$ntakePraexam = isset($hist[1][$id]) ? $hist[1][$id] : 0;
		$ntakeMateri = isset($hist[0][$id]) ? $hist[0][$id] : 0;
		...
		$this->mysmarty->assign("lcourse_taken_praexam", sprintf(getconfig('lcourse_taken'),$ntakePraexam));
		$this->mysmarty->assign("lcourse_taken_materi", sprintf(getconfig('lcourse_taken'),$ntakeMateri));

		$this->db->where("general_setting_code", "showtrainingcounterpraexammateri");
		$q = $this->db->get("general_setting");
		if ($q->num_rows() == 0) {
			$this->mysmarty->assign("showCounterPraexamMateri", 0);
		} else {
			$row =$q->row();
			$this->mysmarty->assign("showCounterPraexamMateri", $row->general_setting_value);
		}
