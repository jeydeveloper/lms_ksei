...
class Request extends Base {
				...
				foreach ($jabatan_approval as $key => $value) {
					...
					if(is_array($tmp_jabatan)) {
						foreach ($tmp_jabatan as $value2) {
							...
							$res = $this->db->select('user_id')->join('jabatan', 'user_jabatan = jabatan_id')->where('jabatan_name = "'.$value2.'"')->get('user')->row_array();
						...
						$res = $this->db->select('user_id')->join('jabatan', 'user_jabatan = jabatan_id')->where('jabatan_name = "'.$value.'"')->get('user')->row_array();
						$user_id = !empty($res['user_id']) ? $res['user_id'] : 0;
	...
	function formtraining_edit($request_id = '') {
		...
		if(!empty($_POST)) {
			...
			if(!empty($frm['rqtr_participant'])) {
				...
				$res = $this->db->select('user_id, user_npk, user_first_name', false)->where('user_id IN ('.$tmp.')')->get('user')->result_array();
	...
	function do_search_user() {
		...
		$condition = array();
		if(!empty($extra_option)) $condition[] = 'jabatan_name = "'.$extra_option.'"';
		if(!empty($user_keyword)) $condition[] = '(user_first_name LIKE "%'.$user_keyword.'%" OR user_npk LIKE "%'.$user_keyword.'%")';
		...
		if(!empty($condition)) {
			...
			$res = $this->db->from('user')->join('jabatan', 'user_jabatan=jabatan_id', 'LEFT')->join('level_group', 'jabatan_level_group=level_group_id', 'LEFT')->order_by('user_first_name', 'ASC')->get()->result_array();
		}
	...
	function formtraining_edit_setting($request_id = '') {
		...
		if(!empty($_POST)) {
			...
			foreach($frm['trse_user_step_approval'] as $kMain => $vMain) {
				...
				if(!empty($vMain)) {
					$tmp = join(',', $vMain);
					$res = $this->db->select('user_id, user_npk, user_first_name, jabatan_name', false)->join('jabatan', 'user_jabatan=jabatan_id', 'LEFT')->join('level_group', 'jabatan_level_group=level_group_id', 'LEFT')->where('user_id IN ('.$tmp.')')->get('user')->result_array();
					foreach ($res as $key => $value) {
